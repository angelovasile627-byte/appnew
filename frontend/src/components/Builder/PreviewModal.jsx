...
const generateBlockHTML = (config) => {
  ...
  switch (config.type) {
    ...
    case 'custom-form': {
      ...
      const overlayDisplay = config.overlay?.enabled && config.background?.type === 'image' ? 'block' : 'none';
      
      // Field styles
      const fieldStyle = config.customStyle ? `
        padding: 0.5rem;
        background-color: ${config.colors?.fieldColor || '#ffffff'};
        border: 1px solid ${config.colors?.borderColor || '#cccccc'};
      ...
      const hoverStyle = config.customStyle ? `
        .custom-form input:hover,
        .custom-form input:focus,
        .custom-form textarea:hover,
        .custom-form textarea:focus {
          background-color: ${config.colors?.fieldHover || '#efefef'};
          border-color: ${config.colors?.borderHover || '#cccccc'};
          color: ${config.colors?.formFocusText || '#232323'};
        }
        .custom-form input::placeholder,
        .custom-form textarea::placeholder {
          color: ${config.colors?.formPlaceholder || '#c1c1c1'};
        }
        .custom-form input:focus::placeholder,
        .custom-form textarea:focus::placeholder {
          color: ${config.colors?.formFocusPlaceholder || '#c1c1c1'};
        }
      ` : '';
      ...
      return `
        <style>
          ...
          ${config.disableElements ? `
            .custom-form input::-webkit-clear-button,
            .custom-form input::-webkit-inner-spin-button,
            .custom-form input::-webkit-outer-spin-button,
            .custom-form input::-webkit-calendar-picker-indicator {
              display: none;
            }
          ` : ''}
        </style>
        <div class="custom-form" style="
          ...
          background-color: ${config.background?.type === 'color' ? config.background.value : 'transparent'};
          background-image: ${config.background?.type === 'image' && config.background.imageUrl ? `url(${config.background.imageUrl})` : 'none'};
          background-size: cover;
          background-position: center;
        ">
          <div style="
            ...
            right: 0;
            bottom: 0;
            background-color: ${config.overlay?.color || '#ffffff'};
            opacity: ${config.overlay?.opacity || 0.4};
            display: ${overlayDisplay};
            pointer-events: none;
          "></div>
          
          <div style="max-width: 66.666%; margin: 0 auto; position: relative; z-index: 1;">
            ${config.title?.show ? `
              <h4 style="
                text-align: ${config.title?.align || 'center'};
            ...
            ${config.icon?.show ? `
              <div style="text-align: center; margin-bottom: 1rem;">
                <svg version="1.1" xmlns="http://www.w3.org/2000/svg" width="${config.icon?.size || 200}" height="${config.icon?.size || 200}" viewBox="0 0 32 32" fill="${config.icon?.color || '#777777'}">
            ...
            ${config.subtitle?.show ? `
              <p style="
                text-align: ${config.subtitle?.align || 'center'};
            ...
            <form action="${config.formAction || ''}" method="${config.formMethod || 'post'}" style="margin-top: 2rem;">
              <input type="text" placeholder="Name" style="${fieldStyle}" />
              <input type="email" placeholder="Email" style="${fieldStyle}" />
              <textarea placeholder="Message" rows="5" style="${fieldStyle}"></textarea>
