...
def generate_html_from_blocks(blocks: List[Block]) -> str:
    ...
    block_html_map = {
        ...
        "hero-1": '<section class="relative h-screen flex items-center justify-center bg-gradient-to-br from-purple-600 via-indigo-600 to-blue-500 text-white"><div class="text-center z-10 px-4"><h1 class="text-6xl font-bold mb-6">{{headline}}</h1><p class="text-2xl mb-8">{{subheadline}}</p><button class="bg-white text-indigo-600 px-8 py-4 rounded-full text-lg font-semibold hover:bg-gray-100 transition">{{ctaText}}</button></div></section>',
